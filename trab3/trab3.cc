...
public:
	...
	void atualiza()
	{
		cx += velocidade*sin(angulo*M_PI/180.0);
		cy -= velocidade*cos(angulo*M_PI/180.0);
	}
};
...
void mouseclick(int butt, int st, int x, int y)
{
	if((butt == GLUT_LEFT_BUTTON) && (st == GLUT_DOWN) && (ifup))
	{
		...
		municao a = municao(jogador.cx + dx, jogador.cy - dy, RTIRO, jogador.delta+jogador.vacan, veltiro);
		tiros.push_back(a);
		ifup = false;
	}
